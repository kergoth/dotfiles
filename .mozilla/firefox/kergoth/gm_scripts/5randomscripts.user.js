...
function add_randoms() {
  ...
  GM_xmlhttpRequest({
    ...
    onload: function(result) {
      ...
      h1 = res.slice(res.indexOf('<div id="content">')+18,res.indexOf("</h1>",res.indexOf('<div id="content">'))+5);
      if (h1.match("Uh-oh! The page could not be found!") != null) {
        add_randoms();
      } else{
        div = document.createElement("div");
        div.setAttribute("class", "random");
        a = parseInt(res.slice(res.indexOf('<a href="/scripts/source/')+25,res.indexOf(".user.js")));
        div.innerHTML = "<a href = 'scripts/show/"+a+"'>"+h1+"</a>";
        div.innerHTML += res.slice(res.indexOf('</h1>',res.indexOf('<div id="content">'))+5,res.indexOf("</p>")+4);
