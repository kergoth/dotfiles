...
function add_rates(){
   ...
   for ( i = scriptsTable.getElementsByTagName("tr").length - 1; i > 0; i--) {
      ...
      var info = scriptRows.getElementsByTagName("td")[2];
      var installs = scriptRows.getElementsByTagName("p")[2].wrappedJSObject.firstChild.data;
      installs = parseInt(installs.split(" ")[0].replace(",", ""));
      var parts = scriptRows.getElementsByTagName("abbr")[0].getAttribute("title").replace(/[T:Z]/g, "-").split("-");
      ...
      ratep.appendChild(document.createTextNode("~" + (rate100/100) + " installs/day"));
...
function XHR () {
  author = document.location.href.replace(";",".xml;");
  scriptsTable = document.getElementsByTagName("table")[0];
  rateSpace = document.getElementById("content").getElementsByTagName("p")[0].appendChild(document.createElement('span'));
  total_rate = 0;
  GM_xmlhttpRequest({
    method: 'get',
    headers: {
	...
	for (j = scripts.length - 1; j >= 0; j--) {
	  ...
	  if (parseInt(scripts[i].getElementsByTagName('id')[0].textContent) == (a.substring(a.lastIndexOf('/')+1, a.length))) {
            info = scriptsTable.getElementsByTagName("tr")[j+1].getElementsByTagName("td")[2];
