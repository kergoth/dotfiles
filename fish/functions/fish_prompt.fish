...
function fish_set_terminal_app_cwd
    # printf "\e]7;file://%s/%s\a" "localhost" (echo $argv[2] | sed 's/ /%20/g')
    # printf "\e]7;file://%s/%s\a" $argv[1] (echo $argv[2] | sed 's/ /%20/g')
    printf >&2 '\a\033]7;file://%s/%s' $HOSTNAME (echo -n $PWD | sed 's/ /%20/g')
end
