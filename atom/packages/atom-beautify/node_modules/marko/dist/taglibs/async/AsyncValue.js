...
AsyncValue.prototype = {
     ...
     * available.
     */
    bz_: function (callback) {
        // Do we already have data or error?
        if (this.by_) {
            // invoke the callback immediately
            return callback(this.bw_, this.az_);
        }

        var callbacks = this.bx_ || (this.bx_ = []);
    ...
    bB_: function (value) {
        ...
        if (value && typeof value.then === "function") {
            ...
            var finalPromise = value.then(function onFulfilled(value) {
                nextTick(asyncValue.bB_.bind(asyncValue, value));
            }, function onRejected(err) {
                nextTick(asyncValue.bA_.bind(asyncValue, err));
            });
