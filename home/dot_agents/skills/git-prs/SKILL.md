...
- Distill private context into reviewer context. Do not point reviewers at chat logs, local plans, scratch files, uncommitted changes, or workflow narration as sources of truth. If that context contains decisions, constraints, rejected alternatives, risk tradeoffs, review hazards, or verification limits, restate short items directly in the PR body and link to committed or public artifacts for longer detail. Scrub sensitive or internal-only material before copying it into a PR.
- For non-trivial diffs, make the review path explicit. Identify what is mechanical versus semantic, where reviewers should start, what needs close scrutiny, and what evidence was used to validate the change. Reference commits, files, tests, CI, issues, or design docs when they help navigation, but the description should still stand on its own.
