...
PATH="/opt/homebrew/bin:$HOME/.brew/bin:$HOME/.nix-profile/bin:/usr/local/bin:$PATH"
...
COLIMA_LOG_FILE='/tmp/xbar_colima_log'

if [ $# -eq 0 ]
then
    colima_status=
    if [ -f "$COLIMA_STATUS_FILE" ]; then
        colima_status=$(cat "$COLIMA_STATUS_FILE")
    fi
...
then
    echo "starting" >> "$COLIMA_LOG_FILE"
    echo "starting" > "$COLIMA_STATUS_FILE"
    LIMA_SHELL= colima start 2>&1 | tee "$COLIMA_LOG_FILE"
    rm -f "$COLIMA_STATUS_FILE"
    echo "done" >>"$COLIMA_LOG_FILE"
    exit 0
elif [  "$1" = stop ]
then
    echo "stopping" >> "$COLIMA_LOG_FILE"
    echo "stopping" > "$COLIMA_STATUS_FILE"
    colima stop 2>&1 | tee "$COLIMA_LOG_FILE"
    rm -f "$COLIMA_STATUS_FILE"
    echo "done" >> "$COLIMA_LOG_FILE"
    exit 0
fi
