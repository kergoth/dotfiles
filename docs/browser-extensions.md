# Browser Extensions

Snapshots of my browser extension configurations from various browsers.

## Vivaldi Browser

**Browser Engine:** Chromium/Blink
**Extensions:** Chrome Web Store (chrome.google.com/webstore) - Compatible with most Chrome extensions

### Extensions as of 2026-02-07

- [1Password -- Password Manager](https://chrome.google.com/webstore/detail/aeblfdkhhhdcdjpifhhbdiojplfjncoa): The best way to experience 1Password in your browser. Easily sign in to sites, generate passwords, and store secure information.
- [DeArrow - Better Titles and Thumbnails](https://chrome.google.com/webstore/detail/enamippconapkdmgfgjchkhakpfinmaj): Crowdsourcing titles and thumbnails to be descriptive and not sensational
- [Decentraleyes](https://chrome.google.com/webstore/detail/ldpochfccmkkmhdbclfhpagapcfdljkj): Protects you against tracking through centralized content delivery
- [Duplicate Tabs Closer](https://chrome.google.com/webstore/detail/gnmdbogfankgjepgglmmfmbnimcmcjle): Detects and closes duplicate tabs automatically
- [Obsidian Web Clipper](https://chrome.google.com/webstore/detail/cnjifjpddelmedmihgijeibhnjfabmlf): Save and highlight web pages in a private and durable format that you can access offline
- [Readwise Highlighter](https://chrome.google.com/webstore/detail/jjhefcfhmnkfeepcpnilbbkaadhngkbi): Save a page to Readwise Reader
- [SponsorBlock for YouTube - Skip Sponsorships](https://chrome.google.com/webstore/detail/mnjggcdmjocbbbhaepdhchncahnbgone): Skip sponsorships, subscription begging and more on YouTube videos
- [Tab Session Manager](https://chrome.google.com/webstore/detail/iaiomicjabeggjcfkbimgmglanimpnae): Save and restore the state of windows and tabs. It also supports automatic saving.
- [Tampermonkey](https://chrome.google.com/webstore/detail/dhdgffkkebhmkfjojejmpbldmpobfkfo): Change the web at will with userscripts
- [Things To Get Me](https://chrome.google.com/webstore/detail/hikgojdlmopjpjfbmmpoleceddnhdjbi): The universal wish-list maker. Add items to your gift list from any website.
- [Toolkit for YNAB](https://chrome.google.com/webstore/detail/lmhdkkhepllpnondndgpgclfjnlofgjl): UI customizations and tweaks for the web version of YNAB.
- [uBlock Origin](https://chrome.google.com/webstore/detail/cjpalhdlnbpafiamejdnhcphjbkeiagm): Finally, an efficient blocker. Easy on CPU and memory.

### Extensions as of 2026-02-02

#### Enabled

- [1Password -- Password Manager](https://chrome.google.com/webstore/detail/aeblfdkhhhdcdjpifhhbdiojplfjncoa): The best way to experience 1Password in your browser. Easily sign in to sites, generate passwords, and store secure information.
- [Augmented Steam](https://chrome.google.com/webstore/detail/dnhpnfgdlenaccegplpojghhmaamnnfp): Augments your Steam Experience
- [Auto Tab Discard (suspend)](https://chrome.google.com/webstore/detail/jhnleheckmknfcgijgkadoemagpecfol): Increase browser speed and reduce memory load when you have numerous open tabs.
- [ClearURLs](https://chrome.google.com/webstore/detail/lckanjgmijmafbedllaakclkaicjfmnk): Remove tracking elements from URLs.
- [Cookie AutoDelete](https://chrome.google.com/webstore/detail/fhcgjolkccmbidfldomjliifgaodjagh): Control your cookies! Automatically delete unwanted cookies from your closed tabs while keeping the ones you want.
- [CSS Exfil Protection](https://chrome.google.com/webstore/detail/ibeemfhcbbikonfajhamlkdgedmekifo): Guard against CSS data exfiltration attacks.
- [Decentraleyes](https://chrome.google.com/webstore/detail/ldpochfccmkkmhdbclfhpagapcfdljkj): Protects you against tracking through centralized content delivery.
- [Distill Web Monitor](https://chrome.google.com/webstore/detail/inlikjemeeknofckkjolnjbpehgadgge): Monitor webpage or feed for changes. Get SMS and email alerts on change detection.
- [Duplicate Tab Helper](https://chrome.google.com/webstore/detail/oaceoebbkmkgfjhmngdinoclnionlgoh): Prevent duplicate tabs effortlessly.
- [Extension Switch](https://chrome.google.com/webstore/detail/gnphfcibcphlpedmaccolafjonmckcdn): Provides a switch to toggle statuses of installed extensions.
- [Header Editor Lite](https://chrome.google.com/webstore/detail/eningockdidmgiojffjmkdblpjocbhgh): Manage browser requests
- [Kagi Search](https://chrome.google.com/webstore/detail/cdglnehniifkbagbbombnjghhcihifij): Set Kagi as your default search engine while also preserving your session in private browsing.
- [MEGA](https://chrome.google.com/webstore/detail/bigefpfhnfcobdlfbedofhhaibnlghod): Secure cloud storage, chat, and meetings
- [OneTab](https://chrome.google.com/webstore/detail/chphlpgkkbolifaimnlloiipkdnihall): Save up to 95% memory and reduce tab clutter
- [Redirector](https://chrome.google.com/webstore/detail/ocgpenflpmgnfapjedencafcfakcekcd): Automatically redirect content based on user-defined rules.
- [Referer Control](https://chrome.google.com/webstore/detail/hnkcfpcejkafcihlgbojoidoihckciin): Control the HTTP Referer on a per-site basis.
- [Refined GitHub](https://chrome.google.com/webstore/detail/hlepfoohegkhhmjieoechaddaejaokhf): Simplifies the GitHub interface and adds useful features
- [RSS Subscription Extension (by Google)](https://chrome.google.com/webstore/detail/nlbjncdgjeocebhnmkbbbdekmmmcbfjd): Adds one-click subscription to your toolbar.
- [Save Page WE](https://chrome.google.com/webstore/detail/dhhpefjklgkmgeafimnjhojgjamoafof): Save a complete web page as a single HTML file that can be opened in any browser.
- [Shut Up: Comment Blocker](https://chrome.google.com/webstore/detail/oklfoejikkmejobodofaimigojomlfim): Blocks comment sections on many popular websites.
- [Skip Redirect](https://chrome.google.com/webstore/detail/jaoafjdoijdconemdmodhbfpianehlon): Skip intermediary pages that some pages use before redirecting to a final page.
- [Tab Session Manager](https://chrome.google.com/webstore/detail/iaiomicjabeggjcfkbimgmglanimpnae): Save and restore the state of windows and tabs. It also supports automatic saving.
- [Tampermonkey](https://chrome.google.com/webstore/detail/dhdgffkkebhmkfjojejmpbldmpobfkfo): Change the web at will with userscripts
- [Toolkit for YNAB](https://chrome.google.com/webstore/detail/lmhdkkhepllpnondndgpgclfjnlofgjl): UI customizations and tweaks for the web version of YNAB.
- [uBlacklist](https://chrome.google.com/webstore/detail/pncfbmialoiaghdehhbnbhkkgmjanfhe): Blocks sites you specify from appearing in Google search results
- [uBlock Origin](https://chrome.google.com/webstore/detail/cjpalhdlnbpafiamejdnhcphjbkeiagm): Finally, an efficient blocker. Easy on CPU and memory.

#### Disabled

- [Copy All Urls](https://chrome.google.com/webstore/detail/djdmadneanknadilpjiknlnanaolmbfk): Copy tabs URL to clipboard (formats: text, HTML, JSON or custom). Paste to open multiple URL at one go.
- [enhanced-h264ify](https://chrome.google.com/webstore/detail/omkfmpieigblcllmkgbflkikinpkodlk): Choose what video and audio codecs YouTube should play for you
- [Library Extension](https://chrome.google.com/webstore/detail/chkgcmmjoejpekoegkedcpifgfhpjmec): See books, music and more at your local library as you browse the internet
- [Privacy Redirect](https://chrome.google.com/webstore/detail/pmcmeagblkinmogikoikkdjiligflglb): Redirects Twitter, YouTube, Instagram and more to privacy friendly alternatives.
- [Proton VPN: Fast & Secure](https://chrome.google.com/webstore/detail/jplgfhpmjnbigmhklmmbgecoobifkmpa): Secure your internet and protect your online privacy in one click.
- [Push to Kindle](https://chrome.google.com/webstore/detail/pnaiinchjaonopoejhknmgjingcnaloc): Send web articles to your Kindle
- [QR-Code](https://chrome.google.com/webstore/detail/leicjbimaiecjfljbeikdailcmaadlml): Show a QR-Code for the current URL (without relying on some third-party API).
- [Readwise](https://chrome.google.com/webstore/detail/egfepjgjabnppmaiadpedbgadkcelcbd): Use the Readwise Chrome Extension to sync your Kindle highlights and then review them daily.
- [Save to Pocket (mini)](https://chrome.google.com/webstore/detail/calnfcidhlhdmmmnicdpddbmmjmfohem): Adds a toolbar button to save the current page to Pocket.
- [Send to Kindle for Google Chrome™](https://chrome.google.com/webstore/detail/cgdjpilhipecahhcilnafpblkieebhea): Sending and reading web content such as news articles and blog posts to your Kindle device or reading app is now easier than ever.
- [uBlock Origin Scope](https://chrome.google.com/webstore/detail/bbdpgcaljkaaigfcomhidmneffjjjfgp): A tool which reports remote server connections
- [Violentmonkey](https://chrome.google.com/webstore/detail/jinjaccalgkegednnccohejagnlnfdag): An open source userscript manager that supports a lot of browsers
- [xStyle](https://chrome.google.com/webstore/detail/hncgkmhphmncjohllpoleelnibpmccpj): Restyle the web with xStyle, a user styles manager. With xStyle you can install themes & skins for many sites and create your own.
- [Zoom Page WE](https://chrome.google.com/webstore/detail/bcdjhkphgmiapajkphennjfgoehpodpk): Zoom web pages (either per-site or per-tab) using full-page zoom, text-only zoom, automatic fit-to-width and minimum font size.

## Firefox

### Extensions as of 2026-02-07

- [1Password](https://addons.mozilla.org/en-US/firefox/addon/1password-x-password-manager/): The best way to experience 1Password in your browser. Easily sign in to sites, generate passwords, and store secure information.
- [DeArrow - Better Titles and Thumbnails](https://addons.mozilla.org/en-US/firefox/addon/dearrow/): Crowdsourcing titles and thumbnails to be descriptive and not sensational
- [Decentraleyes](https://addons.mozilla.org/en-US/firefox/addon/decentraleyes/): Protects you against tracking through centralized content delivery
- [Duplicate Tabs Closer](https://addons.mozilla.org/en-US/firefox/addon/duplicate-tabs-closer/): Detects and closes duplicate tabs automatically
- [Obsidian Web Clipper](https://addons.mozilla.org/en-US/firefox/addon/web-clipper-obsidian/): Save and highlight web pages in a private and durable format that you can access offline
- [Readwise Highlighter](https://addons.mozilla.org/en-US/firefox/addon/readwise-highlighter/): Save a page to Readwise Reader
- [SponsorBlock for YouTube - Skip Sponsorships](https://addons.mozilla.org/en-US/firefox/addon/sponsorblock/): Skip sponsorships, subscription begging and more on YouTube videos
- [Tab Session Manager](https://addons.mozilla.org/en-US/firefox/addon/tab-session-manager/): Save and restore the state of windows and tabs. It also supports automatic saving.
- [Tampermonkey](https://addons.mozilla.org/en-US/firefox/addon/tampermonkey/): Change the web at will with userscripts
- [Things To Get Me](https://addons.mozilla.org/en-US/firefox/addon/things-to-get-me/): The universal wish-list maker. Add items to your gift list from any website.
- [Toolkit for YNAB](https://addons.mozilla.org/en-US/firefox/addon/toolkit-for-ynab/): UI customizations and tweaks for the web version of YNAB.
- [uBlock Origin](https://addons.mozilla.org/en-US/firefox/addon/ublock-origin/): Finally, an efficient blocker. Easy on CPU and memory.

### Extensions as of 2020-03-08

#### Enabled

- [1Password extension (desktop app required)](https://agilebits.com/onepassword)
- [Augmented Steam](https://es.isthereanydeal.com/)
- [Auto Tab Discard](https://add0n.com/tab-discard.html)
- [Awesome RSS](https://shgysk8zer0.github.io)
- Bear Writer
- [Canonical](https://github.com/bhadaway/canonical)
- [ClearURLs](https://gitlab.com/KevinRoebert/ClearUrls)
- [Containers On The Go](https://addons.mozilla.org/en-US/firefox/addon/containers-on-the-go/)
- [Cookie AutoDelete](https://github.com/Cookie-AutoDelete/Cookie-AutoDelete)
- [CSS Exfil Protection](https://www.mike-gualtieri.com/css-exfil-vulnerability-tester)
- [Decentraleyes](https://decentraleyes.org/)
- [Download Manager (S3)](http://www.s3blog.org/s3download-statusbar.html)
- [Drag-Select Link Text](https://github.com/nkestrel/dragselectlinktext)
- [Duplicate Tabs Closer](https://addons.mozilla.org/en-US/firefox/addon/duplicate-tabs-closer/)
- [Enhancer for YouTube™](https://www.mrfdev.com/enhancer-for-youtube)
- [Firefox Multi-Account Containers](https://github.com/mozilla/multi-account-containers#readme)
- [GitHub Issue Hoister](https://github.com/mcomella/github-issue-hoister)
- [Gmail Quick Links](https://github.com/kevinwucodes/gmail-quick-links)
- [h264ify](https://github.com/erkserkserks/h264ify)
- [Header Editor](http://team.firefoxcn.net)
- [HTTPS Everywhere](https://www.eff.org/https-everywhere)
- [IdentFavIcon Quantum](https://www.github.com/wolfgang42/identfavicon-quantum)
- [Lovely forks](https://github.com/musically-ut/lovely-forks)
- [OneTab](https://www.one-tab.com/)
- [Pinboard Pin](https://github.com/Cito/Pinboard-Pin)
- [Push to Kindle](http://fivefilters.org/kindle-it/)
- Reddit Scammer Labeler
- [Redirector](http://einaregilsson.com/redirector/)
- [Refined GitHub](https://github.com/sindresorhus/refined-github)
- [Save Page WE](https://addons.mozilla.org/en-US/firefox/addon/save-page-we/)
- [Shut Up: Comment Blocker](https://rickyromero.com/shutup/)
- [SiteDelta Watch](http://sitedelta.schierla.de/)
- [Skip Redirect](https://github.com/sblask/webextension-skip-redirect)
- [Smart Referer](https://gitlab.com/smart-referer)
- [Switch Container](https://gitlab.com/mjanetmars/switch-container)
- [Tab Mover](https://code.guido-berhoerster.org/addons/firefox-addons/tab-mover/)
- [Tab Session Manager](https://github.com/sienori/Tab-Session-Manager)
- [Tab Tally](https://addons.mozilla.org/en-US/firefox/addon/tab-tally/)
- [Tab2QR](https://addons.mozilla.org/en-US/firefox/addon/tab2qr/)
- [Tampermonkey](https://www.tampermonkey.net/)
- [Textarea Cache](https://addons.mozilla.org/firefox/addon/textarea-cache/)
- [Toolkit for YNAB](https://github.com/toolkit-for-ynab/toolkit-for-ynab/)
- [Tree Style Tab](http://piro.sakura.ne.jp/xul/_treestyletab.html.en)
- [uBlock Origin](https://github.com/gorhill/uBlock#ublock-origin)
- [WebToEpub](https://github.com/dteviot/WebToEpub): Used for fetching webfiction from royalroad.com for Kindle reading
- [xStyle](http://team.firefoxcn.net)

#### Disabled

- [Awesome Autocomplete for GitHub](https://github.com/algolia/github-awesome-autocomplete)
- [CanvasBlocker](https://github.com/kkapsner/CanvasBlocker/)
- [ColorfulTabs](https://www.addongenie.com/)
- [Download Star](https://addons.mozilla.org/en-US/firefox/addon/download-star/)
- [DownThemAll! beta](https://downthemall.org/)
- Ganbo
- [HTTPS by default](https://github.com/Rob--W/https-by-default)
- [Incognito This Tab](https://browsernative.com/incognito-tab-firefox/)
- [instakeybinds](https://github.com/sbdchd/instakeybinds)
- [Instapaper](https://addons.mozilla.org/en-US/firefox/addon/instapaper-official/)
- Library Extension
- [Load Background Tabs Lazily](https://addons.mozilla.org/en-US/firefox/addon/load-background-tabs-lazily/)
- LoadTabOnSelect Evolution
- [Multiple Tab Handler](http://piro.sakura.ne.jp/xul/_multipletab.html.en)
- [Never Force Download](https://addons.mozilla.org/en-US/firefox/addon/never-force-download/)
- NoSquint Plus
- [Pinboard WebExtension](https://github.com/gapop/pinboard-webextension)
- [Stylus](https://add0n.com/stylus.html)
- [Test Pilot](https://testpilot.firefox.com)
- [TST Colored Tabs](https://github.com/MurzNN/tst-colored-tabs)
- [Violentmonkey](https://violentmonkey.github.io/)
- Wallabagger

## Zen Browser

**Browser Engine:** Firefox/Gecko
**Extensions:** Firefox Add-ons (addons.mozilla.org) - Compatible with all Firefox extensions

### Extensions as of 2026-02-03

#### Enabled

- [1Password](https://addons.mozilla.org/en-US/firefox/addon/1password-x-password-manager/): The best way to experience 1Password in your browser for signing in to sites, generating passwords, and storing secure information
- [Auto Tab Discard](https://addons.mozilla.org/en-US/firefox/addon/auto-tab-discard/): Increase browser speed and reduce memory load by automatically discarding unused tabs
- [Canonical](https://github.com/bhadaway/canonical): Gets the canonical URL for the current page
- [Decentraleyes](https://addons.mozilla.org/en-US/firefox/addon/decentraleyes/): Protects you against tracking through centralized content delivery
- [h264ify](https://addons.mozilla.org/en-US/firefox/addon/h264ify/): Makes YouTube stream H.264 videos instead of VP8/VP9 videos
- [OneTab](https://addons.mozilla.org/en-US/firefox/addon/onetab/): Save up to 95% memory and reduce tab clutter by converting tabs to a list
- [Prevent Duplicate Tabs](https://addons.mozilla.org/en-US/firefox/addon/duplicate-tab-detector/): Prevents opening duplicate tabs
- [Privacy Badger](https://addons.mozilla.org/en-US/firefox/addon/privacy-badger17/): Automatically learns to block hidden trackers
- [Readwise](https://addons.mozilla.org/en-US/firefox/addon/readwise/): Sync your Kindle highlights and review them daily
- [Readwise Highlighter](https://addons.mozilla.org/en-US/firefox/addon/readwise-highlighter/): Save articles to Reader and highlight the open web
- [Tab Session Manager](https://addons.mozilla.org/en-US/firefox/addon/tab-session-manager/): Save and restore the state of windows and tabs with automatic saving
- [uBlock Origin](https://addons.mozilla.org/en-US/firefox/addon/ublock-origin/): An efficient blocker. Easy on CPU and memory

#### Disabled

- [Containers On The Go](https://addons.mozilla.org/en-US/firefox/addon/containers-on-the-go/): Quickly switch container tabs with keyboard shortcuts
- [Cookie AutoDelete](https://addons.mozilla.org/en-US/firefox/addon/cookie-autodelete/): Automatically delete unwanted cookies from closed tabs
- [Duplicate Tabs Closer](https://addons.mozilla.org/en-US/firefox/addon/duplicate-tabs-closer/): Detects and closes duplicate tabs automatically
- [HoldTab](https://addons.mozilla.org/en-US/firefox/addon/holdtab/): Delays new tab loading until you activate it
- [Redirector](https://addons.mozilla.org/en-US/firefox/addon/redirector-2/): Automatically redirect URLs based on user-defined rules
- [Referer Control](https://addons.mozilla.org/en-US/firefox/addon/referercontrol/): Control the HTTP Referer on a per-site basis
- [Request Control](https://addons.mozilla.org/en-US/firefox/addon/requestcontrol/): Control requests with filtering, redirecting, and blocking rules
- [Resurrect Pages](https://addons.mozilla.org/en-US/firefox/addon/resurrect-pages-isup-edition/): View cached or archived versions of dead web pages
- [Shut Up: Comment Blocker](https://addons.mozilla.org/en-US/firefox/addon/shut-up-comment-blocker/): Blocks comment sections on many popular websites
- [Skip Redirect](https://addons.mozilla.org/en-US/firefox/addon/skip-redirect/): Skip intermediary redirect pages automatically
- [Textarea Cache](https://addons.mozilla.org/en-US/firefox/addon/textarea-cache/): Saves text you enter in text areas to prevent data loss

## Chrome

### Extensions as of Unknown Date, Last Synced on 2026-02-03

#### Enabled

- [Clutter Free - Prevent duplicate tabs](https://chrome.google.com/webstore/detail/iipjdmnoigaobkamfhnojmglcdbnfaaf): Reduce tab clutter - prevent duplicate tabs, quickly search & switch tabs, and more...
- [Decentraleyes](https://chrome.google.com/webstore/detail/ldpochfccmkkmhdbclfhpagapcfdljkj): Protects you against tracking through centralized content delivery
- [Download Master - Free Download Manager](https://chrome.google.com/webstore/detail/laepcndcehndnjndpfjdcdgbneoimdgg): Download all files in one click. Free video downloader, image downloader, files downloader that supports all web sites & file types
- [Enhancer for YouTube™](https://chrome.google.com/webstore/detail/ponfpcnoihfmfllpaingbgckeeldkhle): Take control of YouTube and boost your user experience
- [Go Incognito](https://chrome.google.com/webstore/detail/oimfnddepdafdfmammggkfmdepfmipai): Opens current tab in an incognito window, and removes it from your browsing history
- [Google Docs Offline](https://chrome.google.com/webstore/detail/ghbmnnjooekpmoecnnnilnnbdlolhkhi): Edit, create, and view your documents, spreadsheets, and presentations — all without internet access
- [h264ify](https://chrome.google.com/webstore/detail/aleakchihdccplidncghkekgioiakgal): Makes YouTube stream H.264 videos instead of VP8/VP9 videos
- [Header Editor Lite](https://chrome.google.com/webstore/detail/eningockdidmgiojffjmkdblpjocbhgh): Manage browser requests
- [Instapaper](https://chrome.google.com/webstore/detail/ldjkgaaoikpmhmkelcgkgacicjfbofhh): Instapaper browser extension to save links for offline reading
- [Kicktraq](https://chrome.google.com/webstore/detail/mkfbmdekphdknccdlbhlkbifdbdioekd): Track Kickstarter Campaigns
- [OneTab](https://chrome.google.com/webstore/detail/chphlpgkkbolifaimnlloiipkdnihall): Save up to 95% memory and reduce tab clutter
- [Pinboard Plus](https://chrome.google.com/webstore/detail/mphdppdgoagghpmmhodmfajjlloijnbd): A better extension for Pinboard. Easy to know current page has been saved or not
- [Pinboard Tools](https://chrome.google.com/webstore/detail/dpaohcncbmkojcpcjaojcehdlnjfbjkl): Toolbar actions for Pinboard
- [QR-Code](https://chrome.google.com/webstore/detail/leicjbimaiecjfljbeikdailcmaadlml): Show a QR-Code for the current URL (without relying on some third-party API)
- [Rearrange Tabs](https://chrome.google.com/webstore/detail/ccnnhhnmpoffieppjjkhdakcoejcpbga): Allows users to rearrange tabs using keyboard shortcuts
- [Refined GitHub](https://chrome.google.com/webstore/detail/hlepfoohegkhhmjieoechaddaejaokhf): Simplifies the GitHub interface and adds useful features
- [RSS Subscription Extension (by Google)](https://chrome.google.com/webstore/detail/nlbjncdgjeocebhnmkbbbdekmmmcbfjd): Adds one-click subscription to your toolbar
- [Send to Kindle for Google Chrome™](https://chrome.google.com/webstore/detail/cgdjpilhipecahhcilnafpblkieebhea): Sending and reading web content such as news articles and blog posts to your Kindle device or reading app is now easier than ever
- [Session Buddy - Tab & Bookmark Manager](https://chrome.google.com/webstore/detail/edacconmaakjimmfgnblocblbcdcpbko): Keep Tabs
- [Shut Up: Comment Blocker](https://chrome.google.com/webstore/detail/oklfoejikkmejobodofaimigojomlfim): Blocks comment sections on many popular websites
- [SiteDelta Watch](https://chrome.google.com/webstore/detail/bkpoemflpiempcejpkoifiaklcjfkbak): Regularly monitors a website for changes
- [Stylebot](https://chrome.google.com/webstore/detail/oiaejidbmkiecgbjeifoejpgmdaleoha): Change the appearance of the web instantly
- [Stylus](https://chrome.google.com/webstore/detail/clngdbkpkpeebahjckkjfobafhncgmne): Redesign the web with Stylus, a user styles manager. Stylus allows you to easily install themes and skins for many popular sites
- [Tampermonkey](https://chrome.google.com/webstore/detail/dhdgffkkebhmkfjojejmpbldmpobfkfo): Change the web at will with userscripts
- [Toolkit for YNAB](https://chrome.google.com/webstore/detail/lmhdkkhepllpnondndgpgclfjnlofgjl): UI customizations and tweaks for the web version of YNAB
- [WebToEpub](https://chrome.google.com/webstore/detail/akiljllkbielkidmammnifcnibaigelm): Used for fetching webfiction from royalroad.com for Kindle reading

#### Disabled

- [Amazon Smile Redirect](https://chrome.google.com/webstore/detail/ejglonclnjogoiegggjjcpapffbnangg): This Extension will automatically redirect you to the corresponding AmazonSmile page from Amazon.com
- [Auto Tab Discard (suspend)](https://chrome.google.com/webstore/detail/jhnleheckmknfcgijgkadoemagpecfol): Increase browser speed and reduce memory load when you have numerous open tabs
- [Ghostery Tracker & Ad Blocker - Privacy AdBlock](https://chrome.google.com/webstore/detail/mlomiejdfkolichcflejclcbmpeaniij): Powerful and easy-to-use privacy extension. Block ads, trackers & speed up your web
- [Library Extension](https://chrome.google.com/webstore/detail/chkgcmmjoejpekoegkedcpifgfhpjmec): See books, music and more at your local library as you browse the internet
- [PayPal Honey: Automated Coupons & Cash Back](https://chrome.google.com/webstore/detail/bmnlcjabgnpnenekpadlanbbkooimhnj): Save money and earn rewards when you shop online
- [Privacy Badger](https://chrome.google.com/webstore/detail/pkehgijcmpdhfbdbbnkijodmdjhbjlgp): Automatically learns to block hidden trackers. Made by leading digital rights nonprofit EFF to stop companies from spying on you
- [Query stripper](https://chrome.google.com/webstore/detail/ccemjnmiieofdhbjbbidpkjcfimakdio): Strips annoying query parameters from URLs. It does utm_* (google), atl_* (atlassian) and ref_ (imdb). More tokens are customizable
- [Readwise](https://chrome.google.com/webstore/detail/egfepjgjabnppmaiadpedbgadkcelcbd): Use the Readwise Chrome Extension to sync your Kindle highlights and then review them daily
- [Terms of Service; Didn't Read](https://chrome.google.com/webstore/detail/hjdoplcnndgiblooccencgcggcoihigg): Get information about websites' terms of service and privacy policies, with ratings and summaries from ToSDR
- [Tiny Suspender](https://chrome.google.com/webstore/detail/bbomjaikkcabgmfaomdichgcodnaeecf): Unload / suspend / hibernate idle tabs to reduce memory and cpu usage
- [Toby: Tab Management Tool](https://chrome.google.com/webstore/detail/hddnkoipeenegfoeaoibdmnaalmgkpip): Your productivity hub. Save and organize your browser tabs from anywhere
- [User-Agent Switcher for Chrome](https://chrome.google.com/webstore/detail/djflhoibgkdhkhhcedjiklpkjnoahfmg): Spoofs & Mimics User-Agent strings
- [Wizmage Image Hider](https://chrome.google.com/webstore/detail/ifoggbfaoakkojipahnplnbfnhhhnmlp): Wizmage hides images in a way that minimizes the impact on the user's browsing experience
