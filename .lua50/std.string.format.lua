...
-- @func string.numbertobinarysi: Write a number using the binary SI suffixes
-- The number is always written to 3 s.f.
--   @param n: number
-- @returns
--   @param n_: string
function string.numbertobinarysi (n)
  local SIprefix = {
    [0] = "", [1] = "Ki", [2] = "Mi", [3] = "Gi",
    [4] = "Ti", [5] = "Pi", [6] = "Ei"
  }

  local _,xp = math.frexp(n)
  local SIxp = math.floor(xp / 10)
  local man = math.floor(n / 2^(SIxp*10), 2)
  return tostring(man) .. SIprefix[SIxp]
end
